...
class CheckDependencies(object):
    ...
    def getSPARTAdir(self, options):
        ...
        try:
            #Get list of SPARTA_Windows versions on the Desktop
            spartaver = glob.glob("SPARTA_Windows-*")[:]
            #Get latest version of SPARTA_Windows in case multiple versions exist on the Desktop
            spartaver.sort()
            if os.path.lexists(os.path.join(desk_path, spartaver[0])):
                sparta_dir = os.path.join(desk_path, spartaver[0])

            elif os.path.lexists(os.path.join(desk_path, "SPARTA_Windows")):
